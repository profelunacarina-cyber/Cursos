...
Para trabajar con preview/staging, crear además una base separada para `dev`
(`profeluna-dev`). No uses la base de producción para el preview: el aula guarda
datos personales de estudiantes, progreso y certificados.

### 2. Configuración local

```bash
cp .env.example .env.local  # pegar DATABASE_URL dev/local y un JWT_SECRET
npm install
npm run db:init             # aplica migraciones y carga semillas iniciales
...
Para recuperación de contraseña por correo, configurar además `RESEND_API_KEY` y
`EMAIL_FROM`. `EMAIL_FROM` debe pertenecer a un dominio verificado en Resend.

Para producción, usar sólo comandos explícitos contra `.env.production`:

```bash
npm run db:init:prod
npm run admin:crear:prod -- carina@ejemplo.com UnaClaveSegura "Carina Luna"
```
...
2. En *Settings → Environment Variables* cargar `DATABASE_URL`, `JWT_SECRET`,
